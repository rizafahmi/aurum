defmodule AurumWeb.IdleVaultCleanupTest do
  @moduledoc """
  Feature tests for US-106: Idle Vault Cleanup.

  The core idle timeout behavior is tested in `Aurum.VaultDatabase.IdleCleanupTest`
  (unit tests). These feature tests focus on user-observable outcomes when
  using DynamicRepo directly in integration scenarios.

  Note: Regular ConnCase tests use the Ecto sandbox for performance.
  These tests use the :integration tag to test actual DynamicRepo behavior.
  """
  ...
  describe "US-106: Idle Vault Cleanup" do
    ...
    test "no data loss on repo restart" do
      ...
      assert hd(items).name == "Gold Bar Before Idle"
    end

    test "activity resets idle timer" do
      {:ok, vault, _token} = Accounts.create_vault()

      {:ok, pid} = DynamicRepo.start_repo(vault.id, idle_timeout: 150)
      ...
      Process.sleep(100)
      assert Process.alive?(pid), "Process should still be alive after activity reset"

      Process.sleep(200)
      refute Process.alive?(pid), "Process should stop after extended inactivity"
    end
  end
end
